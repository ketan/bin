...
class GitHttp
  class App
    ...
    def call(env)
      # so we can support multi threaded execution
      dup.process(env)
    end
    ...
    def user_info
      ...
      return nil if auth_header.strip.empty?
      user_info = Base64.decode64(auth_header.split(' ').last)
      user_info.split(':')
    end
    ...
    def service_rpc
      ...
      @res.finish do
        ...
        IO.popen(command, File::RDWR) do |pipe|
          # puts "Got input from client: #{input}"
          pipe.write(input)
          while !pipe.eof?
    ...
    NEED_AUTHORIZATION = PLAIN_TYPE.merge("WWW-Authenticate" => 'Basic realm="Active Directory LDAP Authentication"')
